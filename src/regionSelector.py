...
class RegionSelector():
    ...
    def __drawLines(self,cord_x,cord_y):
        ...
        #making blur
        local_frame[:,0:self.__cord_x1+1] = cv2.GaussianBlur(local_frame[:,0:self.__cord_x1+1],(5,5),6)
        local_frame[:,self.__cord_x2-1:self.__width] = cv2.GaussianBlur(local_frame[:,self.__cord_x2-1:self.__width],(5,5),6)
        
        cv2.imshow(self.__title,local_frame)

        print(self.__title+'--->','cord_x1:',self.__cord_x1,'  cord_x2:',self.__cord_x2)


if __name__ == "__main__":
    frame = cv2.imread('data/img/car3.jpg')
